...
### `image-round-crop`
Apply a rounded corner crop to an image, matching the radius detected from a reference file. Outputs PNG with transparent corners (or `#F8F8F8` background for opaque sources).

**Invoke:** `/salesforce/image-utils/image-round-crop`
